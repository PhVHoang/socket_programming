...
/*_Bool addSession (Session *ss, User *us) {
  ...
  int t= indexSession(ss->cliaddr);
  strcpy(session[t].user->id,us->id);
  strcpy(session[t].user->pass,us->pass);
  session[t].status = AUTHENTICATED;
...
/*int indexSession(struct sockaddr_in *cliaddr)
{
	for (int i = 0; i < no_session; i++) {
        if (isEqualSockAddrIn(session[i].cliaddr, cliaddr))
            return i ;
